...
interface EmptyStateProps {
  ...
  description: string;
  actionLabel: string;
  actionHref: Route | URL;
}

export default function EmptyState({ title, description, actionLabel, actionHref }: EmptyStateProps) {
  const actionClassName =
    "rounded-full bg-brand-turquoise px-6 py-3 text-sm font-semibold text-white transition hover:bg-brand-teal";

  const isExternal = actionHref instanceof URL;
  ...
  return (
    <section className="flex flex-col items-center justify-center gap-6 rounded-3xl border border-dashed border-brand-turquoise/40 bg-white/70 p-12 text-center shadow-sm backdrop-blur">
      ...
      {isExternal ? (
        <a href={actionHref.toString()} className={actionClassName} target="_blank" rel="noreferrer">
          {actionLabel}
        </a>
      ) : (
